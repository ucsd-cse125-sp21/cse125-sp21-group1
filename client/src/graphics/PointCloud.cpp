	...
	: pointSize(pointSize)
{
    ...
    if(!ifs.is_open()) {std::cerr<<"failed reading file\n"; exit(1);}
    ...
    while(std::getline(ifs, l)){
        ...
        if(type == "v") {
            iss >> x >> y >> z;
            if (maxx < x) maxx = x; if (maxy < y) maxy = y; if (maxz < z) maxz = z;
            if (minx > x) minx = x; if (miny > y) miny = y; if (minz > z) minz = z;
            upscaled_points.push_back(glm::vec3(x, y, z));
            lightType.push_back(pointSize);
        }
        else if(type == "vn") {
            iss >> x >> y >> z;
//            vn.push_back(0.5f* glm::normalize(glm::vec3(x, y, z)) + 0.5f);
            vn.push_back(glm::vec3(x, y, z));
        }
        else if (type == "f") {
            iss >> x1; iss.get(); iss.get(); iss >> x2;
            iss >> y1; iss.get(); iss.get(); iss >> y2;
            iss >> z1; iss.get(); iss.get(); iss >> z2;
            face.push_back(--x1); face.push_back(--y1); face.push_back(--z1);
        }
    }
    ifs.close();
    ...
    // center all points
    auto disx = (maxx + minx) / 2, disy = (maxy + miny) / 2, disz = (maxz + minz) / 2;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * vn.size(),
            vn.data(), GL_STATIC_DRAW);
